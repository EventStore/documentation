...
After clicking on the `Create cluster` button, the provisioning process starts. The cluster will immediately be visible in the list of clusters. If you have opted to create an `IP Access List`, it will be created followed by the cluster itself. After a few minutes, your new cluster will be ready to use.
