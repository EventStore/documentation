...
::: tip
If you enabled [enabled security](connecting-to-a-server.md), reading metadata may require that you pass credentials. By default it is only allowed for admins though you can change this via default ACLs. If you do not pass credentials and they are required you will receive an `AccessedDeniedException`.
:::
...
This method will put the data that is in metadata as the stream metadata. Metadata in this case can be anything in a vector of bytes. The server only understands JSON. Read [Access Control Lists](../security/acl.md) for more information on the format in JSON for access control lists.
