...
Subscriptions allow you to subscribe to a stream and receive notifications when new events are added.

You provide an event handler and an optional starting point for the subscription. The handler is called for each event from the starting point onward.
...
### Subscribing from a specific position in a stream
...
This won't read through the history of the stream but will instead notify the handler when a new event appears in the respective stream.
...
::: tip
[Filtered subscriptions](subscriptions.md#server-side-filtering) make it easier and faster to subscribe to all events of a specific type or those with matching prefixes.
:::

When reading a stream you can specify whether to resolve link-to's or not. By default, link-to events are not resolved. You can change this behavior by setting the `resolveLinkTos` parameter to `true`:
...
The `subscriptionDropped` callback allows you to inspect why the subscription dropped and any exceptions.

The possible reasons for a subscription to drop are:

| Reason            | Why it might happen                                                                                                  |
|:------------------|:---------------------------------------------------------------------------------------------------------------------|
| `Disposed`        | The subscription was canceled or disposed of by the client.                                                            |
...
Bear in mind that a subscription can also drop because it is slow. The server tries to push all the live events to the subscription while in live processing mode. If the subscription gets the reading buffer overflow and won't be able to acknowledge the buffer, it will break.

### Handling subscription drops

An application that hosts the subscription can go offline for a period of time for different reasons. It could be a crash, infrastructure failure, or a new version deployment. As you rarely would want to reprocess all the events again, you'd need to store the current position of the subscription somewhere and then use it to restore the subscription from the point where it dropped off:
...
The code below shows how you can provide user credentials for a subscription. When you specify subscription credentials explicitly, it will override the default credentials set for the client. If you don't specify any credentials, the client will use the credentials specified for the client, if specified.
...
::: tip
Server-side filtering was introduced as a simpler alternative to projections. To get the events you care about, consider filtering before creating a projection.
:::
...
There are several events in EventStoreDB called system events. These are prefixed with a `$`; you won't care about these under most circumstances. They can be filtered out by passing in a `SubscriptionFilterOptions` when subscribing to the `$all` stream.
...
::: tip
`$stats` events are no longer stored in EventStoreDB by default, so the number of `$` events will be limited.
:::

### Filtering by event type

If you only want to subscribe to events of a given type, there are two options:  A regular expression or a prefix.
...
This will only subscribe to events with a type that begins with `customer-.`
...
There are two options if you only want to subscribe to a stream with a given name. You can either use a regular expression or a prefix.
...
When a catch-up subscription is used to process an `$all` stream containing a large number of events, the last thing you want is for your application to crash midway, forcing you to restart from the beginning.

### What is a checkpoint?

A checkpoint is the position of an event in the `$all` stream that your application has processed. By saving this position to a persistent store (e.g., a database), it allows your catch-up subscription to:
...
To create a checkpoint, you should store either the event's commit or prepare position.
...
The checkpoint notification is sent after every 32 non-system events processed from $all by default.
...
By configuring this parameter, you provide a balance between reducing checkpoint overhead and ensuring quick recovery in case of a failure.
