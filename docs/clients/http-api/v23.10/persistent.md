...
::: warning 
Persistent subscriptions to `$all` are not supported over the HTTP API. If you want to update persistent subscriptions to `$all`, use the [appropriate client method](@server/persistent-subscriptions.md#updating-a-subscription-group). 
:::
