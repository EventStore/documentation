...
<p>It is possible to make an optimistic concurrency check during the append operation by specifying the version at which you expect the stream to be. Identical append operations are idempotent if the optimistic concurrency check is not disabled. You can find more information on optimistic concurrency and idempotence <RouterLink to="/clients/dotnet/20.10/appending/optimistic-concurrency-and-idempotence.html">here</RouterLink>.</p>
...
<p>@[code](clients/dotnet/20.10/sample-code/DotNetClient/AppendStreamEvents.cs#AppendOneEvent</p>
<p>Example list of events in single write:</p>
<p>@[code](clients/dotnet/20.10/sample-code/DotNetClient/AppendStreamEvents.cs#CreateSampleData</p>
