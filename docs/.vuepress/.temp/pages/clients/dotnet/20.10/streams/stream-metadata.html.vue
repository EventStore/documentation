...
<tr>
<td style="text-align:left"><code>long MetastreamVersion</code></td>
<td style="text-align:left">The version of the metastream (see <RouterLink to="/clients/dotnet/20.10/streams/optimistic-concurrency-and-idempotence.html">Expected Version</RouterLink>)</td>
</tr>
<tr>
...
<p>If you enabled <RouterLink to="/clients/dotnet/20.10/streams/connecting-to-a-server.html">enabled security</RouterLink>, reading metadata may require that you pass credentials. By default it is only allowed for admins though you can change this via default ACLs. If you do not pass credentials and they are required you will receive an <code>AccessedDeniedException</code>.</p>
</div>
<h2 id="writing-metadata" tabindex="-1"><a class="header-anchor" href="#writing-metadata" aria-hidden="true">#</a> Writing metadata</h2>
<p>You can write metadata in both a typed and a raw mechanism. When writing it is generally easier to use the typed mechanism. Both writing mechanisms support an <code>expectedVersion</code> which works the same as on any stream and you can use to control concurrency, read <RouterLink to="/clients/dotnet/20.10/streams/optimistic-concurrency-and-idempotence.html">Expected Version</RouterLink> for further details.</p>
...
</code></pre><div class="line-numbers"><span class="line-number">1</span><br></div></div><p>This method will put the data that is in metadata as the stream metadata. Metadata in this case can be anything in a vector of bytes. The server only understands JSON. Read <RouterLink to="/clients/dotnet/20.10/security/acl.html">Access Control Lists</RouterLink> for more information on the format in JSON for access control lists.</p>
<div class="custom-container tip"><p class="custom-container-title">TIP</p>
