...
</div><p>It is possible to make an optimistic concurrency check during the append by specifying the version at which you expect the stream to be. Identical append operations are idempotent if the optimistic concurrency check is not disabled. You can find more information on optimistic concurrency and idempotence <RouterLink to="/clients/dotnet/21.2/optimistic-concurrency-and-idempotence.html">here</RouterLink>.</p>
