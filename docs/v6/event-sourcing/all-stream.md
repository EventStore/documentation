...
In that case, the subscription that feeds the projection with events has to subscribe to some stream that contains all the events from entities of different types, with different ids. 
...
Probably the most useful thing about the global event stream is that all events in it are perfectly ordered and any subscription that uses the `$all` stream will get events exactly in the same order as they were written to Event Store, even if events were appended to different logical streams. That way projections can be reassured that things that must happen in order will do so, as long as the write-side of the application behaves correctly.
...
For example, following previous illustrations and snippets, our eCommerce system would one day require a page that shows all the orders for one single customer. It could be possible to build an API endpoint that runs a query for that purpose. There are drawbacks for adding queries though since each query potentially introduces side effects on the database, like space used for indexes, degraded performance and so on. Some databases won't even let you query without changing the persistence model significantly. Building a new read model, however, is relatively straightforward.

![ProjectionNewAggregation](./images/projection-aggregation.png)

One thing to remember there is all the events that are included in such aggregation need to have a field that conveys the aggregation id (customer id in this case). It might be problematic if the original event schema didn't contain enough data. Albeit not exactly straightforward, but this problem has several solutions, like stream joins (enrichment), upcasting, migration and so on. Those patterns are out of scope for this article.
